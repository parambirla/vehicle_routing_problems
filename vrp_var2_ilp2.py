...
def deadline_vrp_lp_gurobi(n,start_time ,end_time, t, S,active_customers,service_time,disc_fact):
    ...
    for v in active_customers:
        if(v!=S):
            x_inc = []
            for i in vert_custi[v]:
                for j in edges_in[(v,i)]:
                    x_inc.append(x[(j,(v,i))])
                    
            model.addConstr(gp.quicksum(x_inc) == 1, name=f"visit_{v}")
...
filename = "D:/college/sem_10/cod892/vrp_code/test_cases/solomon/R107.txt"
