...
def deadline_vrp_lp_gurobi(n,start_time,end_time,service_time,demands, C,t, S,active_customers,disc_fact,disc_demand_fact):
    ...
    '''
    #print(len(vertices))
    #2.  x_e for each vertex (v,i) , sum of incoming  = sum of outgoing 
    for v,i,j in vertices:
            incomingx = [x[(e,(v, i, j))] for e in edges_in[(v, i, j)]]
            outgoingx = [x[((v, i, j),e)] for e in edges_out[(v, i, j)] ]     
            model.addConstr(gp.quicksum(incomingx) == gp.quicksum(outgoingx))

    # (v,t,j)  , sum of served demands * x >=demand_i
    for v in active_customers:
        incomingx = [x[e]*(e[1][2]-e[0][2]) for e in edges if e[1][0]==v]
        for i in vert_custi[v]:
            #for k in range(C+1):
            k=0
            while(k<=C):
                for e in edges_in[(v,i,k)]:
                    incomingx.append(x[(e,(v,i,k))]*(k-e[2]))
                k=k+disc_demand_fact
                    
        model.addConstr(gp.quicksum(incomingx)>=demands[v])
...
'''
def solve_all_customers_iteratively(n, d,demands,C, t, S):
    all_customers = set(range(n))
    all_customers.remove(S)  # exclude depot from visiting set

    unsatisfied_customers = set(all_customers)
    full_visited_set = []
    round_num = 0
    objval=0.0
    #print(unvisited_customers)
    start_time = time.time()
    curr_demands=demands
    while len(unsatisfied_customers)>0:
        print(f"\n--- Iteration {round_num} ---")
        #active_customers = list(unvisited_customers.union({S}))  # include S each time
        active_customers = list(unsatisfied_customers)
        (sats,rem_dem) = deadline_vrp_lp_gurobi(n, d, curr_demands,C,t, S, active_customers)

        if not sats:
            print("Infeasible or no customers visited in this iteration. Stopping.")
            break

        print(f"satisfied in iteration {round_num}: {sats}")
        full_visited_set.append(sats)

        # Remove visited customers from unvisited list
        unsatisfied_customers.difference_update(sats)
        curr_demands=rem_dem
...
solve_all_customers_iteratively(n,deadlines,demands,C,t,S)
