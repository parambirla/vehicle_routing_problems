...
def deadline_vrp_lp_gurobi(n, d, t, S,active_customers):
    ...
    # Objective  
    objective = gp.quicksum(x[e] for e in edges if e[0] == (S,0))
    ...
    for v in active_customers:
        if(v!=S):
            sink_node = (v, d[v] + 1)
            incoming_edges = [e for e in edges if e[1] == sink_node]
            model.addConstr(gp.quicksum(flow[(v, e)] for e in incoming_edges) == 1, name=f"flow_to_sink_{v}")

    #incoming x_e for every customer = 1 
    for v in active_customers:
        if(v!=S):
            target_vertices = [(v, i) for i in range(d[v] + 1)]
            incoming_edges = [e for e in edges if e[1] in target_vertices]
            model.addConstr(gp.quicksum(x[e] for e in incoming_edges) == 1, name=f"visit_customer_{v}")
...
filename = "test_cases/n_40/t1.txt"
...
def solve_all_customers_iteratively(n, d, t, S):
    ...
    while len(unvisited_customers)>0:
        ...
        visited = deadline_vrp_lp_gurobi(n, d, t, S, active_customers)
...
solve_all_customers_iteratively(n,deadlines,t,S)
